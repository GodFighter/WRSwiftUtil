...
private struct wr_activity_associated{
   static var managerKey = "wr_activity_associated_manager_key"
}

public extension WRActivityIndicatorProtocol {
        
    var wr : WRActivityIndicatorManager {
        get {
            return WRActivityIndicatorManager.shared
        }
    }
    
    var isAnimating: Bool {
        return self.wr.isAnimating
    }

    
    func stopAnimating(_ fadeOutAnimation: WRFadeOutAnimation? = WRActivityIndicatorView.DEFAULT_FADE_OUT_ANIMATION) {
        self.wr.stopAnimating(fadeOutAnimation)
    }
...
@objc public class WRActivityIndicatorManager : NSObject {
    ...
    public static let shared = WRActivityIndicatorManager()
    ...
    public final func startAnimating(_ data: WRActivityData, _ fadeInAnimation: WRFadeInAnimation? = nil) {
        self.data = data
        state.startAnimating(manager: self, fadeInAnimation)
    }

    public final func stopAnimating(_ fadeOutAnimation: WRFadeOutAnimation? = nil) {
        state.stopAnimating(manager: self, fadeOutAnimation)
    }
